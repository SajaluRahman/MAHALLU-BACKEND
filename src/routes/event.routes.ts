...
r.post('/', authorize(PERMISSIONS.EVENT_CREATE), async (req: AuthRequest, res, next) => { try { const e = await Event.create({ ...req.body, tenantId: req.user!.tenantId }); res.status(201).json({ success: true, data: e }); } catch (e) { next(e); } });
